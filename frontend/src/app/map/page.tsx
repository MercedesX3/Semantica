      ...
      <div className="flex-1 relative overflow-hidden">
        <div className="absolute top-4 left-4 z-20">
          <SearchInput
            ...
            placeholder="Find a book on the map"
          />
        </div>
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          {CLUSTER_BOOKS.map((book) => {
            ...
            const bg = book.cluster === "romance"
              ? isCenter ? "bg-pink-500" : "bg-pink-300"
              : "bg-blue-400";
            const outlineClass = isSelected ? "outline-[3px] -outline-offset-[3px] outline-pink-500" : "outline-2 -outline-offset-2 outline-black";
            ...
            return (
              ...
              >
                <div
                  className={`rounded-full ${bg} ${outlineClass} flex items-center justify-center transition-all`}
          ...
          {selected && (
            <div className="absolute right-4 top-4 w-72 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] outline outline-1 outline-offset-[-1px] outline-black overflow-hidden z-10">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {DETAIL_BOOK.genome.map((g, i) => (
                      <span key={i} className="h-8 px-4 py-1 rounded-md outline-2 -outline-offset-2 outline-black bg-white text-sm font-semibold font-mono inline-flex items-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                        {g}
                      </span>
                    ))}
                  </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-1 mt-1">
                            {related.genres.map((g, j) => (
                              <span key={j} className="h-6 px-3 rounded-md outline-2 -outline-offset-2 outline-black bg-white text-xs font-semibold font-mono inline-flex items-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                {g}
                              </span>
                            ))}
                          </div>
